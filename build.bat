...
set INCLUDE=%SDK_DIR%\Include\%SDK_VERSION%\km;%WDK_DIR%\Include\%SDK_VERSION%\um;%WDK_DIR%\Include\wdf\kmdf\%WDK_VERSION%
set LIB=%SDK_DIR%\Lib\%SDK_VERSION%\km\x64;%SDK_DIR%\Lib\%SDK_VERSION%\um\x64;%WDK_DIR%\Lib\wdf\kmdf\x64\%WDK_VERSION%

:: Compilation
cl.exe /D NDIS_MINIPORT_DRIVER /D _X86_ /D WIN32_LEAN_AND_MEAN /D _WIN32_WINNT=0x0A00 /I %INCLUDE% /Zi /W4 /WX /MD /c src\entrypoint.c /FoMyDriver.obj
